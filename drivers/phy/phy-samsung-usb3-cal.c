...
void samsung_exynos_cal_usb3phy_enable(struct exynos_usbphy_info *usbphy_info)
{
	...
	u32 refclkfreq = usbphy_info->refclk;
	u32 phyutmi;
	u32 phytest;
	u32 phyclkrst;
	u32 phyparam0;
	...
	/* disable all q-act by HW detection */
	if (version == 0x0101) {
		phy_resume = readl(regs_base + EXYNOS5_DRD_PHYRESUME);
		...
		writel(phy_resume, regs_base + EXYNOS5_DRD_PHYRESUME);
		udelay(1);
		phy_resume = readl(regs_base + EXYNOS5_DRD_PHYRESUME);
		phy_resume |= PHYRESUME_FORCE_QACT;
		writel(phy_resume, regs_base + EXYNOS5_DRD_PHYRESUME);
	...
	if (version == 0x0101)
		phyreg0 = readl(regs_base + EXYNOS5_DRD_PHYREG0);
	...
	case FSEL_CLKSEL_DIFF_100M:
		...
		if (version != 0x0101) {
			...
			phyreg0 &= ~EXYNOS5_DRD_PHYREG0_SSC_REFCLKSEL_MASK;
			phyreg0 |= EXYNOS5_DRD_PHYREG0_SSC_REFCLKSEL(0x00);
		}
		break;
	case FSEL_CLKSEL_DIFF_26M:
		phyclkrst &= ~PHYCLKRST_MPLL_MULTIPLIER_MASK;
		phyclkrst |= PHYCLKRST_MPLL_MULTIPLIER_26M_REF;
		phyclkrst &= ~PHYCLKRST_REF_CLKDIV2;
		if (version != 0x0101) {
			phyclkrst &= ~PHYCLKRST_SSC_REFCLKSEL_MASK;
			...
			phyreg0 &= ~EXYNOS5_DRD_PHYREG0_SSC_REFCLKSEL_MASK;
			phyreg0 |= EXYNOS5_DRD_PHYREG0_SSC_REFCLKSEL(0x108);
		}
		break;
	...
	case FSEL_CLKSEL_24M:
		...
		if (version != 0x0101) {
			...
			phyreg0 &= ~EXYNOS5_DRD_PHYREG0_SSC_REFCLKSEL_MASK;
			phyreg0 |= EXYNOS5_DRD_PHYREG0_SSC_REFCLKSEL(0x88);
		}
		break;
	default:
		break;
...
void samsung_exynos_cal_usb3phy_crport_handshake(
...
void samsung_exynos_cal_usb3phy_set_cr_port(struct exynos_usbphy_info *usbphy_info)
{
	void __iomem *regs_base = usbphy_info->regs_base;

	/* Set los_bias to 0x5 and los_level to 0x9 */
	samsung_exynos_cal_usb3phy_crport_ctrl(regs_base, 0x15, 0xA409);

	/* Set TX_VBOOST_LEVLE to default Value (0x4) */
	samsung_exynos_cal_usb3phy_crport_ctrl(regs_base, 0x12, 0x8000);

	/* to set the charge pump proportional current */
	samsung_exynos_cal_usb3phy_crport_ctrl(regs_base, 0x30, 0xC0);

	/* Set RXDET_MEAS_TIME[11:4] to 24MHz clock value: 0x80 */
	samsung_exynos_cal_usb3phy_crport_ctrl(regs_base, 0x1010, 0x80);
}
...
void samsung_exynos_cal_usb3phy_tune_dev(struct exynos_usbphy_info *usbphy_info)
{
	...
	if (version == 0x0101) {
		...
		phyparam0 |= PHYPARAM0_TXVREFTUNE(0xa);
		...
		phyparam0 &= ~PHYPARAM0_TXVREFTUNE_MASK;
		phyparam0 |= PHYPARAM0_TXVREFTUNE(0xc);
	}
	writel(phyparam0, regs_base + EXYNOS5_DRD_PHYPARAM0);

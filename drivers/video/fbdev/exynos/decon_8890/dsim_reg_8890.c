...
{
	dsim_write_mask(id, DSIM_SWRST, ~0, DSIM_SWRST_DPHYRST);
	udelay(10);
	dsim_write_mask(id, DSIM_SWRST, 0, DSIM_SWRST_DPHYRST);
}
...
static void dsim_reg_enable_lane(u32 id, u32 lane, u32 en)
{
	...
	dsim_write_mask(id, DSIM_CONFIG, val, DSIM_CONFIG_LANE_EN(lane));
}
...
{
	u32 val = DSIM_CMD_CONFIG_MULTI_CMD_PKT_CNT(DSIM_MULTI_PACKET_CNT);

	dsim_write_mask(id, DSIM_CMD_CONFIG, ~0, DSIM_CMD_CONFIG_MULTI_CMD_PKT_EN);
	dsim_write_mask(id, DSIM_CMD_CONFIG, val, DSIM_CMD_CONFIG_MULTI_CMD_PKT_MASK);
}

void dsim_reg_cmd_ctrl_vsync_based_transfer(u32 id, u32 en)
{
	u32 val = en ? ~0 : 0;
...
void dsim_reg_set_shadow(u32 id, u32 en)
{
	...
	dsim_write_mask(id, DSIM_SFRCTRL, val, DSIM_SHADOW_EN);
}

void dsim_reg_shadow_update(u32 id)
{
	dsim_write_mask(id, DSIM_SFRCTRL, ~0, DSIM_SHADOW_UPDATE);
}
...
void dsim_reg_set_porch(u32 id, struct decon_lcd *lcd)
{
	...
	if (dsim_read_mask(id, DSIM_SFRCTRL, DSIM_SHADOW_UPDATE))
		dsim_reg_shadow_update(id);
}
...
void dsim_reg_set_config(u32 id, u32 mode, u32 data_lane_cnt)
{
	...
	val |= DSIM_CONFIG_BURST_MODE | DSIM_CONFIG_HSE_DISABLE |
		DSIM_CONFIG_NUM_OF_DATA_LANE(data_lane_cnt - 1) | DSIM_CONFIG_MULTI_PIXEL_MODE |
		DSIM_CONFIG_PIXEL_FORMAT(DSIM_PIXEL_FORMAT_RGB24);

	mask = DSIM_CONFIG_NONCONTINUOUS_CLOCK_LANE | DSIM_CONFIG_CLKLANE_STOP_START |
		DSIM_CONFIG_BURST_MODE | DSIM_CONFIG_VIDEO_MODE | DSIM_CONFIG_HSA_DISABLE |
		DSIM_CONFIG_MULTI_PIXEL_MODE |
...
{
	u32 val = en ? ~0 : 0;
	dsim_write_mask(id, DSIM_SFRCTRL, val, DSIM_STAND_BY);
}
...
static int dsim_reg_calculate_pms(struct dsim_pll_param *pll)
{
	...
	u32 fin = 26;
	/* 3HA2 of 8890, Should be PMS(4, 131, 1) */
...
void dsim_reg_init(u32 id, struct decon_lcd *lcd_info, u32 data_lane_cnt)
{
	...
	dsim_reg_cmd_ctrl_vsync_based_transfer(id, 1);
...
int dsim_reg_prepare_clocks(struct dsim_clks_param *clks_param)
{
	...
	dsim_dbg("calculated P(%d),M(%d),S(%d) and HS clock(%d)\n",
...
{
	u32 val = DSIM_CMD_CONFIG_PKT_SEND_CNT(count);
	dsim_write_mask(id, DSIM_CMD_CONFIG, val, DSIM_CMD_CONFIG_PKT_SEND_CNT_MASK);
}
#endif
